...
1. Concurrency Hardening (Optional but Recommended)

* Require If-Match on all mutating requests:

  * Reject PUT/POST without If-Match with a clear error (missing\_if\_match).

* Test:

  * Tab A: HEAD → ETag1, Tab B: PUT → ETag2, Tab A: PUT with ETag1 → 412, refresh and retry → 200.

1. Console Integration Check

* Confirm write URL points to workers.dev origin (REMOTE\_DB\_JSON) and reads come from assets (ASSETS\_REMOTE\_DB\_JSON).
